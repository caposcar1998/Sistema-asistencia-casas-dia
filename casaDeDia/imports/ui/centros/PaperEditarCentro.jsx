...
export default function PaperEditarCentro({centroServidor,centroSeleccionado, handleCerrarEditarCentro}) {
    ...
    useEffect(() => {
        setNombre(centroSeleccionado.nombre);
        setCalle(centroSeleccionado.calle);
        setDelegacion(centroSeleccionado.delegacion);
        setCodigoPostal(centroSeleccionado.codigoPostal);
        setnumeroTelefonico(centroSeleccionado.numeroTelefonico);
        setImage(centroSeleccionado.foto);
    }, []);
        ...
        return new Promise(
            (resolve, reject) => {
                Meteor.call("editarCentro",
                    centroSeleccionado._id,nombre, calle, delegacion,codigoPostal,numeroTelefonico,image,
                    (err, res) => {
                        if (err) {
                            setAlert("error")
                            setSnackBarState(true)
                            setMessage("Error Editar Centro")
                            handleCerrarEditarCentro()
                            reject()
                        } else {
                            setAlert("success")
                            setSnackBarState(true)
    ...
    return (
                <>                    
                        <Grid container>         
                            ...
                            <Grid item xs={12} />
                                <Grid item xs={12}>
                                <Button variant="contained" onClick={handleCerrarEditarCentro} color="secondary">Cancelar</Button>
                                <Button variant="contained" onClick={editarCentro} color="primary">Editar Centro</Button>                        
                                </Grid> 
