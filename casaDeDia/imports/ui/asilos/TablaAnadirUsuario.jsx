...
export default function TablaAnadirPersonal({ asilosServidor,asiloSeleccionado, handleCerrarAnadirUsuario }) {
    ...
    function eliminarUsuario(idEliminar) {
        return new Promise(
            (resolve, reject) => {
                Meteor.call("borrarUsuarioDeAsilo",
                    idEliminar,
                    (err, res) => {
                        if (err) {
                            setAlert("error")
        ...
        <>
            ...
            <TableContainer component={Paper}>
                <Table className={classes.table} aria-label="simple table">
                    ...
                    <TableBody>
                        {asiloSeleccionado.usuarios.map((usuario) => (
                            <UsuariosAsilos
                                usuario={usuario}
...
function CrearNuevoUsuario({ asilosServidor,asiloSeleccionado, handleCerrarAnadirUsuario }) {
    ...
    function crearUsuario() {
        return new Promise(
            (resolve, reject) => {
                Meteor.call("anadirAdultoAsilo",
                    asiloSeleccionado._id, personaSeleccionada._id, personaSeleccionada.nombre, personaSeleccionada.curp,
                    (err, res) => {
                        if (err) {
                            setAlert("error")
                            setSnackBarState(true)
